  ...
  return (
    <div style={themeVars} className={`min-h-screen ${bgGlobal} ${textGlobal} p-4 md:p-8 font-sans relative overflow-x-hidden transition-colors duration-500`}>
      ...
      <AnimatePresence mode="wait">
        ...
        {/* =========================================================
            SESSÃO DE SERVIÇOS (VISUAL ORIGINAL RESTAURADO)
            ========================================================= */}
        {view === 'servicos' && (
        ...
        {deleteModal.isOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className={`p-8 rounded-[32px] w-full max-w-sm shadow-2xl text-center border ${isDarkMode ? 'bg-[#09090b] border-white/10' : 'bg-white border-slate-200'}`}>
              ...
              <h3 className="text-xl font-black uppercase text-red-500 mb-2">Atenção!</h3>
              <p className={`text-sm mb-6 ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>Tem certeza que deseja remover este {deleteModal.type}? Essa ação não pode ser desfeita.</p>
              <div className="flex gap-3">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
             <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className={`p-8 rounded-[32px] w-full max-w-md shadow-2xl border ${isDarkMode ? 'bg-[#09090b] border-white/10' : 'bg-white border-slate-200'}`}>
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="text-xl font-black uppercase text-orange-500">Exclusão em Cadeia</h3>
                    <p className={`text-xs font-bold uppercase tracking-widest ${isDarkMode ? 'text-zinc-500' : 'text-slate-400'}`}>Existem agendamentos vinculados</p>
                  </div>
                </div>
                <p className={`text-sm mb-6 ${isDarkMode ? 'text-zinc-300' : 'text-slate-600'}`}>
                  Você está tentando remover o {bulkDeleteModal.type} <span className="font-bold text-[var(--theme-main)]">{bulkDeleteModal.item?.name}</span>, mas existem <span className="font-black text-red-500">{bulkDeleteModal.relatedApps.length} agendamentos</span> vinculados a ele.<br/><br/>
                  Se você prosseguir, <strong>TODOS esses agendamentos serão excluídos</strong> também. Tem certeza absoluta disso?
                </p>
                <div className="flex gap-3">
                  ...
                  <button onClick={confirmBulkDelete} className="w-full flex-1 bg-red-500 text-white px-4 py-3 rounded-xl font-black text-xs uppercase hover:bg-red-600 cursor-pointer">Deletar Tudo</button>
                </div>
             </motion.div>
          </motion.div>
        )}

        {/* Modal de Sumário (Avisa quantos foram deletados) */}
        {summaryModal.isOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
             <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className={`p-8 rounded-[32px] w-full max-w-sm shadow-2xl text-center border ${isDarkMode ? 'bg-[#09090b] border-white/10' : 'bg-white border-slate-200'}`}>
                ...
                <button onClick={() => setSummaryModal({ isOpen: false, type: '', cancelledApps: [] })} className="w-full bg-[var(--theme-main)] text-[#09090b] px-4 py-3 rounded-xl font-black text-xs uppercase hover:scale-105 transition-transform cursor-pointer">Entendido</button>
             </motion.div>
          </motion.div>
        )}

        {/* Modal de Auto-Confirmação de Fim de Atendimento */}
        {autoConfirmModal.isOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-4 sm:p-0">
             <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className={`p-6 sm:p-8 rounded-[32px] w-full max-w-sm shadow-2xl border ${isDarkMode ? 'bg-[#09090b] border-white/10' : 'bg-white border-slate-200'}`}>
                ...
                <div className="flex gap-2">
                   ...
                   <button onClick={() => { handleFinalizar(autoConfirmModal.appointment.id); setAutoConfirmModal({ isOpen: false, appointment: null }); }} className="flex-1 bg-[var(--theme-main)] text-[#09090b] px-3 py-3 rounded-xl font-black text-[10px] uppercase hover:scale-105 transition-transform cursor-pointer">Finalizar</button>
                </div>
             </motion.div>
          </motion.div>
        )}
